...
from src.model_inference import (
    load_models,
    get_ensemble_segmentation_with_uncertainty,
...
def process_case(case_num, data_dir, results_dir, models, device):
    ...
    nnunet_segmentation_path = os.path.join(
        results_dir,
        f"segmentations/BraTS2021_{case_num}/BraTS2021_{case_num}_nnunet_segmentation.nii.gz",
    )
    if not os.path.exists(nnunet_segmentation_path):
        raise FileNotFoundError(
            f"nnUNet segmentation result not found at: {nnunet_segmentation_path}"
        )
    ...
    ground_truth_path = os.path.join(
        data_dir, f"BraTS2021_{case_num}", f"BraTS2021_{case_num}_seg.nii.gz"
    )
    ground_truth = nib.load(ground_truth_path).get_fdata()

    # Call the ensemble segmentation function
    segmentation, variance_uncertainty, dice_uncertainty = (
        get_ensemble_segmentation_with_uncertainty(
    ...
    img_path = os.path.join(
        data_dir, f"BraTS2021_{case_num}", f"BraTS2021_{case_num}_t1ce.nii.gz"
    )
    ...
    save_segmentation_as_nifti(
        segmentation,
        img_path,
        os.path.join(case_folder, f"{case_num}_ensemble_segmentation.nii.gz"),
    )
