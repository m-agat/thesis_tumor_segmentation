...
def generate_filtered_folder_structure(
    start_path,
    prefix="",
    ...
    directories = [
        item for item in items if os.path.isdir(os.path.join(start_path, item))
    ]
    ...
    for i, directory in enumerate(directories[:max_subfolders_per_folder]):
        pointer = (
            pointers[1]
            if i == len(directories[:max_subfolders_per_folder]) - 1 and not files
            else pointers[0]
        )
        ...
        if current_depth < depth_limit:
            new_prefix = (
                f"{prefix}    "
                if i == len(directories[:max_subfolders_per_folder]) - 1
                else f"{prefix}│   "
            )
            generate_filtered_folder_structure(
                os.path.join(start_path, directory),
                new_prefix,
    ...
    if len(other_files) > max_files_per_folder or (
        show_nii_files and len(nii_gz_files) > max_files_per_folder
    ):
        print(f"{prefix}└── ...")
