...
def process_case(case_num, data_dir, results_dir, models, device):
    ...
    # Load nnUNet segmentation result
    ...
    # Initialize Dice scores dictionary including nnUNet
    ...
    # Compute Dice scores for nnUNet
    for tissue_type in [1, 2, 3]:
...
def main():
    ...
    # Ensure the model comparison directory exists
    ...
    # Limit to 3 cases (as you requested)
    case_nums = case_nums[:3]

    # Load models on CPU
    device = torch.device("cpu")  # Force all models to run on CPU
    ...
    csv_path = os.path.join(model_comparison_dir, 'dice_scores.csv')
