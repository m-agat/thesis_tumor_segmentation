...
def val_epoch(model, loader, epoch, acc_func, model_inferer=None, post_softmax=ls.post_softmax, post_pred=ls.post_pred):
    model.eval()
    start_time = time.time()
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(loader):
            ...
            val_output_convert = [post_pred(post_softmax(val_pred_tensor)) for val_pred_tensor in val_outputs_list]
            ...
            print(
                f"Val {epoch}/{config.config.max_epochs} {idx}/{len(loader)}",
