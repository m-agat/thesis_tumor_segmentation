...
def cross_validate_trainer(
    ...
    for fold in range(num_folds):
        ...
        fold_log_dir = os.path.join(
            config.output_dir,
            f"runs/{optimizer_name}_lr_{initial_lr}_wd_{weight_decay_value}_fold_{fold + 1}",
        )
    ...
    avg_metrics = {
        key: np.nanmean(
            [
                (
                    fold[key][-1]
                    if isinstance(fold[key], (list, np.ndarray))
                    else fold[key]
                )
    ...
    avg_metrics["avg_sensitivity"] = np.nanmean(
        [fold["sensitivity"] for fold in fold_results]
    )
    avg_metrics["avg_specificity"] = np.nanmean(
        [fold["specificity"] for fold in fold_results]
    )
