...
class_weights = pd.read_csv(
    "/home/magata/results/metrics/model_performance_summary.csv", index_col=0
).to_dict(orient="index")
normalized_class_weights = {}
for tissue_index in [1, 2, 4]:
    tissue_key = f"Dice_{tissue_index}"
    tissue_weights = [
        class_weights[model_name][tissue_key] for model_name in class_weights
    ]
...
for idx, batch_data in enumerate(config.test_loader):
    ...
    for model, model_inferer in models_inferers.items():
        mean_pred_ttd, var_pred_ttd = ttd_variance(
            model, image, model_inferer, n_iterations=20
        )
        mean_pred_tta, var_pred_tta = tta_variance(
            model_inferer, image, config.device, n_iterations=20
        )

        entr_mean_pred_ttd, entropy_ttd = ttd_entropy(
            model, image, model_inferer, n_iterations=20
        )
        entr_mean_pred_tta, entropy_tta = tta_entropy(
            model_inferer, image, config.device, n_iterations=20
        )
        ...
        combined_uncertainty_raw = (
            ...
            + weight_ttd_entropy
            * np.repeat(entropy_ttd[np.newaxis, ...], repeats=3, axis=0)
            + weight_tta_entropy
            * np.repeat(entropy_tta[np.newaxis, ...], repeats=3, axis=0)
        )
        combined_uncertainty = (
            combined_uncertainty_raw - np.min(combined_uncertainty_raw)
        ) / (np.max(combined_uncertainty_raw) - np.min(combined_uncertainty_raw))
        print(f"Combined uncertainty shape: {combined_uncertainty.shape}")

        # Get the predicted classes
        combined_prediction_tensor = 0.5 * torch.sigmoid(
            torch.tensor(mean_pred_ttd, device=config.device)
        ) + 0.5 * torch.sigmoid(torch.tensor(mean_pred_tta, device=config.device))
        combined_prediction_tensor = (
            combined_prediction_tensor
            + torch.tensor(entr_mean_pred_ttd, device=config.device)
            + torch.tensor(entr_mean_pred_tta, device=config.device)
        ) / 3
    ...
    )  # Shape: (num_models, tissues, height, width, depth)
    ...
    weighted_probabilities = np.sum(
        weights * ensemble_predictions, axis=0
    )  # Shape: (3, height, width, depth)
    ...
    ensemble_save_path = os.path.join(
        config.output_dir, f"{patient_path[0]}_ensemble_segmentation.nii.gz"
    )
    ...
    uncertainty_map[seg[0] == 1] = total_uncertainty[0][
        seg[0] == 1
    ]  # NCR/NET (Tumor Core)
    uncertainty_map[seg[2] == 1] = total_uncertainty[2][
        seg[2] == 1
    ]  # ET (Enhancing Tumor)
    ...
    uncertainty_map_ncr[seg[0] == 1] = total_uncertainty[0][
        seg[0] == 1
    ]  # NCR (Tumor Core)
    uncertainty_map_ed[seg[1] == 1] = total_uncertainty[1][
        seg[1] == 1
    ]  # ED (Whole Tumor)
    uncertainty_map_et[seg[2] == 1] = total_uncertainty[2][
        seg[2] == 1
    ]  # ET (Enhancing Tumor)

    # Save individual uncertainty maps as NIfTI files
    uncertainty_save_path_ncr = os.path.join(
        config.output_dir, f"{patient_path[0]}_uncertainty_map_NCR.nii.gz"
    )
    uncertainty_save_path_et = os.path.join(
        config.output_dir, f"{patient_path[0]}_uncertainty_map_ET.nii.gz"
    )
    uncertainty_save_path_ed = os.path.join(
        config.output_dir, f"{patient_path[0]}_uncertainty_map_ED.nii.gz"
    )
