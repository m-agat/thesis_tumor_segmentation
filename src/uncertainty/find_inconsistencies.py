...
swinunetr_scores = pd.read_csv(
    "/home/magata/results/metrics/patient_performance_scores_swinunetr.csv"
)
segresnet_scores = pd.read_csv(
    "/home/magata/results/metrics/patient_performance_scores_segresnet.csv"
)
attunet_scores = pd.read_csv(
    "/home/magata/results/metrics/patient_performance_scores_attunet.csv"
)
vnet_scores = pd.read_csv(
    "/home/magata/results/metrics/patient_performance_scores_vnet.csv"
)

swinunetr_scores = swinunetr_scores.rename(
    columns=lambda col: col if col == "Patient" else f"{col}_m1"
)
segresnet_scores = segresnet_scores.rename(
    columns=lambda col: col if col == "Patient" else f"{col}_m2"
)
attunet_scores = attunet_scores.rename(
    columns=lambda col: col if col == "Patient" else f"{col}_m3"
)
vnet_scores = vnet_scores.rename(
    columns=lambda col: col if col == "Patient" else f"{col}_m4"
)
