...
for idx, batch_data in enumerate(config.test_loader_patient):
    ...
    for model, model_inferer in models_inferers.items():
        mean_pred, var_pred = test_time_augmentation_inference(model_inferer, image, config.device, n_iterations=3)
        ...
        cell_type_uncertainty[predicted_labels[1] == 1] = voxel_uncertainties[1][predicted_labels[1] == 1] # ED (Whole Tumor)
        cell_type_uncertainty[predicted_labels[0] == 1] = voxel_uncertainties[0][predicted_labels[0] == 1] # NCR/NET (Tumor Core)
        cell_type_uncertainty[predicted_labels[2] == 1] = voxel_uncertainties[2][predicted_labels[2] == 1] # ET (Enhancing Tumor)  
    ...
    ensemble_save_path = os.path.join(config.output_dir, f"{patient_path[0]}_tta_ensemble_segmentation.nii.gz")
