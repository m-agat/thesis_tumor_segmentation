...
for _, row in performance_data.iterrows():
    ...
    dice_ncr, dice_ed, dice_et = map(lambda x: safe_metric(row[x]), ["Dice_1", "Dice_2", "Dice_4"])
    composite_ncr, composite_ed, composite_et = map(lambda x: safe_metric(row[x]), ["Composite_Score_1", "Composite_Score_2", "Composite_Score_4"])
    hd95_ncr, hd95_ed, hd95_et = map(lambda x: safe_metric(row[x], default=1e6), ["HD95_1", "HD95_2", "HD95_4"])
    sensitivity_ncr, sensitivity_ed, sensitivity_et = map(lambda x: safe_metric(row[x]), ["Sensitivity_1", "Sensitivity_2", "Sensitivity_4"])

    # Weighted metrics for Tumor Core
    if total_tc_count > 0:
        tc_metrics["Dice"].append((dice_ncr * ncr_count + dice_et * et_count) / total_tc_count)
        tc_metrics["Composite_Score"].append((composite_ncr * ncr_count + composite_et * et_count) / total_tc_count)
        tc_metrics["HD95"].append((hd95_ncr * ncr_count + hd95_et * et_count) / total_tc_count)
        tc_metrics["Sensitivity"].append((sensitivity_ncr * ncr_count + sensitivity_et * et_count) / total_tc_count)
    else:
    ...
    if total_wt_count > 0:
        wt_metrics["Dice"].append((dice_ncr * ncr_count + dice_ed * ed_count + dice_et * et_count) / total_wt_count)
        wt_metrics["Composite_Score"].append((composite_ncr * ncr_count + composite_ed * ed_count + composite_et * et_count) / total_wt_count)
        wt_metrics["HD95"].append((hd95_ncr * ncr_count + hd95_ed * ed_count + hd95_et * et_count) / total_wt_count)
        wt_metrics["Sensitivity"].append((sensitivity_ncr * ncr_count + sensitivity_ed * ed_count + sensitivity_et * et_count) / total_wt_count)
    else:
