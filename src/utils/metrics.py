...
def compute_hd95(prediction, ground_truth, tissue_label):
    ...
    hd = compute_hausdorff_distance(
        pred_tissue, gt_tissue, include_background=False, percentile=95, directed=False
    )

    return hd.item()


def compute_metrics_with_monai(
    prediction, ground_truth, tissue_label, confusion_metric
):
    """
...
def calculate_composite_score(dice, hd95, f1_score, weights):
    ...
    return (
        weights["Dice"] * dice + weights["HD95"] * hd95_score + weights["F1"] * f1_score
    )
