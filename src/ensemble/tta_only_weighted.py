...
from uncertainty.test_time_dropout import minmax_uncertainties
...
def ensemble_segmentation_tta(
    test_loader, models_dict, composite_score_weights, n_iterations=10, patient_id=None, output_dir="./output_segmentations/hybrid"
):
    """
    ...
    with torch.no_grad():
        for batch_data in test_data_loader:
            ...
            for model_name, (model, inferer) in models_dict.items():
                tta_mean, uncertainty_vol = tta_variance(inferer, image, config.device, n_iterations=n_iterations)
                uncertainty_vol = minmax_uncertainties(uncertainty_vol)

                # Remove batch dimension => shape becomes (num_classes, H, W, D)
                tta_mean = np.squeeze(tta_mean, axis=0)
                ...
                for ch in range(tta_mean.shape[0]):
...
if __name__ == "__main__":
    ...
    ensemble_segmentation_tta(config.test_loader, models_dict, composite_score_weights, n_iterations=10, patient_id=patient_id)
