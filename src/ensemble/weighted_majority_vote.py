...
results_save_path = os.path.join(
    config.output_dir, f"patient_performance_scores_ensemble.csv"
)
...
with torch.no_grad():
    for idx, batch_data in enumerate(config.test_loader_patient):
        ...
        individual_case_predictions = np.array(
            individual_case_predictions, dtype=np.int32
        ...
        weighted_votes = np.zeros(
            (num_classes,) + individual_case_predictions[0].shape, dtype=np.float32
        ...
        for tissue_type in class_labels:
            ...
            for model_idx, model_pred in enumerate(individual_case_predictions):
                ...
                weighted_votes[class_vote_idx] += (
                    model_pred == tissue_type
                ) * normalized_class_weights[tissue_type][model_name]
        ...
        print(
            f"Saved ensemble segmentation for patient {patient_path[0]} at {save_path}"
        )
        ...
        for tissue_type in class_labels:
            ...
            try:
                ...
                if np.isnan(hd95):
                    print(
                        f"    HD95 for tissue {tissue_type} not computable, setting to 0"
                    )
                    hd95 = float("inf")
            ...
            try:
                ...
                print(
                    f"    Composite score for tissue {tissue_type}: {composite_score}"
                )
            except Exception as e:
                print(
                    f"    Error computing Composite Score for tissue {tissue_type}: {e}"
                )
        ...
        if idx % save_interval == 0:
            ...
            pd.DataFrame(patient_scores).to_csv(
                ...
                index=False,
                mode="a",
                header=not os.path.exists(results_save_path),
            )
