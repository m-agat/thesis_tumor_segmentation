    ...
    with torch.no_grad(), autocast(device_type='cuda', dtype=torch.float16):
        ...
        for batch in data_loader:
            ...
            for name, pred in preds.items():
                ...
                weighted = torch.stack([prob[i] * perf_weights[REGIONS[i]][name] for i in range(len(REGIONS))])
                fused_prob = weighted if fused_prob is None else fused_prob + weighted
            fused_prob = fused_prob / fused_prob.sum(dim=0, keepdim=True)
            seg = fused_prob.argmax(dim=0)
            ...
            save_nifti(fused_prob.cpu(), ref_path, os.path.join(out_dir, f"softmax_{pid_str}.nii.gz"), out_dtype=np.float32)
...
if __name__ == "__main__":
    ...
    ensemble_segmentation(test_loader, models_dict, weights,
                          n_iter=10, pid=None, out_dir="./output/ttd")
