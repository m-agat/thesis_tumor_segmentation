...
def ensemble_segmentation_ttd(
    ...
    n_iterations=10,
    patient_id=None,
    output_dir="./output_segmentations/ttd_only_5",
):
    """
    ...
    with torch.no_grad():
        for batch_data in test_data_loader:
            ...
            for model_name, (model, inferer) in models_dict.items():
                ttd_mean, uncertainty_vol = ttd_variance(
                ...
                for ch in range(ttd_mean.shape[0]):
                    ...
                    effective_weight[ch] = perf_w * np.exp(
                        -alpha * (uncertainty_vol[ch] + epsilon)
                    )
            ...
            output_path = os.path.join(
                output_dir, f"ttd_only_5_segmentation_{patient_id}.nii.gz"
            )
