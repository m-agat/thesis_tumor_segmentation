...
data_path = '/home/agata/Desktop/thesis_tumor_segmentation/data/brats2021challenge/TrainingData'
...
for part in participants:
    ...
    flair_path = os.path.join(participant_folder, [file for file in modalities if "flair" in file][0])
    t1_path = os.path.join(participant_folder, [file for file in modalities if "t1" in file and "t1ce" not in file][0])
    t1ce_path = os.path.join(participant_folder, [file for file in modalities if "t1ce" in file][0])
    t2_path = os.path.join(participant_folder, [file for file in modalities if "t2" in file][0])
    segmentation_path = os.path.join(participant_folder, [file for file in modalities if "seg" in file][0])
    ...
    def get_max_nonzero_slice(image):
        nonzero_counts = [np.count_nonzero(image[:, :, i]) for i in range(image.shape[2])]
        return np.argmax(nonzero_counts)

    # # Get the slice index with the maximum non-zero pixels for each modality
    slice_index = get_max_nonzero_slice(segmentation)  # same slice index used across modalities for consistency
    ...
    for i, (image, title) in enumerate(zip([flair, t1, t1ce, t2, segmentation], titles)):
        plt.subplot(1, 5, i + 1)
        ...
        if title == 'Segmentation':
            ...
            colored_segmentation = np.zeros(segmentation_slice.shape + (3,), dtype=np.float32)
    ...
    legend_elements = [Patch(facecolor=color, edgecolor='k', label=labels[label]) for label, color in colors.items()]
    plt.legend(handles=legend_elements, loc='center left', bbox_to_anchor=(1.05, 0.5), fontsize='large', title="Segmentation Key")
