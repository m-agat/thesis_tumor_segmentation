...
def print_memory_usage():
    ...
    if torch.cuda.is_available():
        print(
            f"GPU Memory: {torch.cuda.memory_allocated() / torch.cuda.max_memory_allocated() * 100:.2f}%"
        )
...
results_save_path = os.path.join(
    config.output_dir, f"patient_performance_scores_{config.model_name}.csv"
)
        ...
        for tissue_type in class_labels:
            ...
            try:
                sensitivity, specificity, f1_score = compute_metrics_with_monai(
                    seg_out, ground_truth, tissue_type, confusion_metric
                )
