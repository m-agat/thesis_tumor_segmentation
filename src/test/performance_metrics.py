...
with torch.no_grad():
    for idx, batch_data in enumerate(config.val_loader):
        try:
            ...
            seg = (prob[0].detach().cpu().numpy() > 0.5).astype(np.int8)
            seg_out = np.zeros(
            ...
            # ground_truth = np.zeros(ground_truth.shape[1:], dtype=np.int8)
            # ground_truth[ground_truth[0] == 1] = 1  # NCR
            # ground_truth[ground_truth[1] == 1] = 2  # ED
            # ground_truth[ground_truth[2] == 1] = 4  # ET

            patient_metrics = {"Patient": patient_path[0]}
            for tissue_type in [1, 2, 4]:
                assert len(tissue_roc_data[tissue_type]['true']) == len(tissue_roc_data[tissue_type]['pred']), \
                    f"Inconsistent lengths for tissue type {tissue_type}: " \
                    f"{len(tissue_roc_data[tissue_type]['true'])} vs {len(tissue_roc_data[tissue_type]['pred'])}"
            ...
            # Save patient scores periodically
            if idx % 10 == 0:
                pd.DataFrame(patient_scores).to_csv(
                    os.path.join(
                        base_path,
                        f"partial_patient_scores_{config.model_name}_{idx}.csv",
                    ),
                    index=False,
                )
...
pd.DataFrame(avg_scores).to_csv(
    os.path.join(base_path, f"average_scores_{config.model_name}.csv"), index=False
)
