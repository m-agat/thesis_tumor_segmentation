...
from transforms import train_transform_wt_binary, val_transform_wt_binary


def read_data_from_folders_binary_wt(train_folder, val_folder, basedir):
    """
...
def get_loader_wt_binary(batch_size, data_dir, train_folder, val_folder, global_roi):
    """
    Create data loaders for WT binary segmentation.
    """
    # Load train and validation files
    train_files, val_files = read_data_from_folders_binary_wt(train_folder, val_folder, data_dir)
    ...
    val_transform_wt_binary = val_transform_wt_binary()

    # Create datasets with WT binary labels
    train_ds = data.Dataset(data=train_files, transform=global_transform_wt_binary)
    val_ds = data.Dataset(data=val_files, transform=val_transform_wt_binary)
