...
train_counts = train_data.groupby(["fold", "region_combination"]).size().reset_index(name="count")
val_counts = val_data.groupby(["fold", "region_combination"]).size().reset_index(name="count")

print(test_data.groupby(["region_combination"]).size().reset_index(name="count"))

# Pivot data for heatmap
heatmap_data = train_counts.pivot(index="region_combination", columns="fold", values="count").fillna(0)
...
for ax, (name, data) in zip(axes, [("Train", train_data), ("Validation", val_data), ("Test", test_data)]):
    sns.countplot(y="region_combination", data=data, ax=ax, order=data["region_combination"].value_counts().index)
