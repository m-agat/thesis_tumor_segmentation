...
brats_ucsf_pdgm = brats_mapping_df[
    brats_mapping_df["Data Collection (as on TCIA+additional)"] == "UCSF-PDGM"
]

# Filter UCSF-PDGM patients only in the "Training" cohort
brats_ucsf_pdgm_training = brats_ucsf_pdgm[
    brats_ucsf_pdgm["Segmentation (Task 1) Cohort"] == "Training"
]
...
merged_training_df = pd.merge(
    brats_ucsf_pdgm_training,
    ucsf_pdgm_df,
    left_on="BraTS2021 ID",
    right_on="BraTS21 ID",
    how="inner",
)

# Analyze tumor types in the training cohort
glioblastoma_count = merged_training_df[
    merged_training_df["Final pathologic diagnosis (WHO 2021)"].str.contains(
        "Glioblastoma", case=False, na=False
    )
].shape[0]
astrocytoma_count = merged_training_df[
    merged_training_df["Final pathologic diagnosis (WHO 2021)"].str.contains(
        "Astrocytoma", case=False, na=False
    )
].shape[0]
...
tumor_grade_distribution = (
    merged_training_df.groupby(
        ["Final pathologic diagnosis (WHO 2021)", "WHO CNS Grade"]
    )
    .size()
...
for _, row in tumor_grade_distribution.iterrows():
    print(
        f"{row['Final pathologic diagnosis (WHO 2021)']} | Grade {row['WHO CNS Grade']} | {row['Count']}"
    )
